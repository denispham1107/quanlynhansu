...
const processWorkSupervisionNowCallable = httpsCallable(
  functions,
...
const els = {
  ...
  employeeWorkSupervisionBanner: $("#employeeWorkSupervisionBanner"),
  employeeWorkSupervisionMessage: $("#employeeWorkSupervisionMessage"),
  employeeWorkSupervisionCountdown: $("#employeeWorkSupervisionCountdown"),
...
let serviceWorkerRegistrationPromise = window.__CULAO_PWA_INSTALL__?.serviceWorkerPromise || null;
...
function registerServiceWorker() {
  ...
  if (serviceWorkerRegistrationPromise) {
    return Promise.resolve(serviceWorkerRegistrationPromise).then((registration) => {
      if (registration) state.pushServiceWorkerRegistration = registration;
      return registration;
    });
  }
...
let deferredPwaInstallPrompt = window.__CULAO_PWA_INSTALL__?.promptEvent || null;
let pwaInstallPromptWaiters = [];
let pwaInstallCompletionHandled = false;
...
function isChromiumBrowser() {
  ...
  return /Chrome|CriOS|Edg|EdgiOS|SamsungBrowser/i.test(ua)
...
function takePwaInstallPrompt() {
  const earlyState = window.__CULAO_PWA_INSTALL__;
  const promptEvent = deferredPwaInstallPrompt || earlyState?.promptEvent || null;
  if (!promptEvent) return null;

  deferredPwaInstallPrompt = null;
  if (earlyState) earlyState.promptEvent = null;
  return promptEvent;
}

function resolvePwaInstallPromptWaiters(promptEvent) {
  const waiters = pwaInstallPromptWaiters;
  pwaInstallPromptWaiters = [];
  waiters.forEach((resolve) => resolve(promptEvent));
}

function waitForPwaInstallPrompt(timeoutMs = 1800) {
  const available = deferredPwaInstallPrompt || window.__CULAO_PWA_INSTALL__?.promptEvent;
  if (available) return Promise.resolve(available);

  return new Promise((resolve) => {
    const timer = window.setTimeout(() => {
      pwaInstallPromptWaiters = pwaInstallPromptWaiters.filter((item) => item !== finish);
      resolve(null);
    }, timeoutMs);
    ...
    pwaInstallPromptWaiters.push(finish);
  });
}
...
function refreshPwaInstallButton() {
  ...
  if (isStandaloneWebApp() || window.__CULAO_PWA_INSTALL__?.installed) {
  ...
  if (isIosDevice()) {
    button.textContent = "Cài đặt trên iPhone/iPad";
    return;
  }

  const promptReady = Boolean(
    deferredPwaInstallPrompt || window.__CULAO_PWA_INSTALL__?.promptEvent
  );
  button.textContent = promptReady ? "Cài đặt ứng dụng" : "Cài đặt";
}

function showIosPwaInstallGuide() {
  setPwaInstallHelp(
    "<strong>iPhone/iPad:</strong> Chrome trên iOS không được phép mở hộp cài đặt PWA. "
      + "Hãy mở đúng địa chỉ này bằng <strong>Safari</strong>, bấm nút <strong>Chia sẻ</strong>, "
      + "chọn <strong>Thêm vào Màn hình chính</strong>, rồi bấm <strong>Thêm</strong>. "
      + "Sau đó Culao Task sẽ chạy dưới dạng ứng dụng độc lập."
  );
}

function showAndroidPwaInstallPendingGuide() {
  setPwaInstallHelp(
    "<strong>Chrome đang kiểm tra điều kiện cài đặt ứng dụng.</strong> "
      + "Hãy giữ trang mở khoảng 30 giây và chạm vào trang ít nhất một lần. "
      + "Khi nút đổi thành <strong>“Cài đặt ứng dụng”</strong>, bấm nút để mở hộp cài đặt chính thức. "
      + "Nếu ứng dụng đã từng được cài, hãy gỡ bản cũ trước."
  );
}

function showDesktopPwaInstallGuide() {
  setPwaInstallHelp(
    "<strong>Trình duyệt chưa cấp hộp cài đặt.</strong> "
      + "Hãy dùng Chrome hoặc Edge ổn định, giữ trang mở một lúc rồi bấm lại. "
      + "Bạn cũng có thể mở menu trình duyệt và chọn <strong>Cài đặt ứng dụng</strong>."
  );
}

function handlePwaInstallPromptReady(event) {
  const promptEvent = event?.type === "beforeinstallprompt"
    ? event
    : window.__CULAO_PWA_INSTALL__?.promptEvent;
  ...
  if (promptEvent) {
    deferredPwaInstallPrompt = promptEvent;
    resolvePwaInstallPromptWaiters(promptEvent);
  }
...
window.addEventListener("beforeinstallprompt", handlePwaInstallPromptReady);
window.addEventListener("culao:pwa-install-ready", handlePwaInstallPromptReady);

function handlePwaInstalled() {
  if (pwaInstallCompletionHandled) return;
  pwaInstallCompletionHandled = true;
  deferredPwaInstallPrompt = null;
  if (window.__CULAO_PWA_INSTALL__) {
    window.__CULAO_PWA_INSTALL__.promptEvent = null;
    window.__CULAO_PWA_INSTALL__.installed = true;
  }
  resolvePwaInstallPromptWaiters(null);
  refreshPwaInstallButton();
  toast("Đã cài đặt Culao Task dưới dạng ứng dụng web.", "success");
}

window.addEventListener("appinstalled", handlePwaInstalled);
window.addEventListener("culao:pwa-installed", handlePwaInstalled);
...
async function openPwaInstallPrompt() {
  let promptEvent = takePwaInstallPrompt();
  ...
  if (!promptEvent) {
    ...
    try {
      ...
      // Việc đăng ký service worker sẽ được kiểm tra lại bằng sự kiện cài đặt.
    }

    await waitForPwaInstallPrompt(1200);
    promptEvent = takePwaInstallPrompt();
  }
...
els.installPwaBtn?.addEventListener("click", async () => {
  ...
  setButtonLoading(els.installPwaBtn, true, "Đang kiểm tra cài đặt...");
  setPwaInstallHelp("");

  try {
    const opened = await openPwaInstallPrompt();
    if (!opened) {
      if (isAndroidDevice() && isChromiumBrowser()) {
        showAndroidPwaInstallPendingGuide();
      } else {
        showDesktopPwaInstallGuide();
      }
    }
  } catch (error) {
...
function isScheduledWorkOrderPhotoRequirementLockedInEditor() {
  if (!state.editingWorkOrderId) return false;
  return state.workOrders.some((workOrder) => (
    workOrder.id === state.editingWorkOrderId
    && workOrder.scheduledWorkOrder === true
  ));
}
...
function syncHotelPhotoRequirementControls() {
  ...
  const scheduledPhotoLock = isScheduledWorkOrderPhotoRequirementLockedInEditor();
  ...
  els.photoRequirementBox?.classList.toggle("is-disabled-by-hotel", hasHotel || scheduledPhotoLock);
  if (els.photoRequiredCheckbox) {
    els.photoRequiredCheckbox.disabled = hasHotel || scheduledPhotoLock;
    if (hasHotel) els.photoRequiredCheckbox.checked = false;
  }
  if (els.requiredPhotoCount) {
    els.requiredPhotoCount.disabled = hasHotel || scheduledPhotoLock || !els.photoRequiredCheckbox?.checked;
  }
...
function updateScheduledCountdownPreview() {
  if (!els.scheduledWorkOrderCountdownPreview) return;
  const minutes = normalizeScheduledCountdownMinutes(els.scheduledWorkOrderCountdownMinutes?.value);
  els.scheduledWorkOrderCountdownPreview.textContent = `${minutes} phút`;
}
...
async function createScheduledWorkOrder(button) {
  ...
  try {
    ...
    toast(
      ...
      + `, đếm ngược ${assignmentCountdownMinutes} phút${repeatText}.`,
      "success"
    );
  } catch (error) {
    console.error(error);
...
function formatScheduledListDateTime(milliseconds) {
...
function renderScheduledWorkOrderList() {
  ...
  els.scheduledWorkOrderList.innerHTML = schedules.map((schedule) => {
    ...
    const title = `${schedule.name || "Phiếu công việc"} - ${taskNames}`;
...
async function openScheduledWorkOrderListModal() {
  ...
  try {
    ...
    console.error(error);
    if (els.scheduledWorkOrderList) {
      els.scheduledWorkOrderList.innerHTML = '<div class="empty-state">Không tải được danh sách lịch. Vui lòng thử lại.</div>';
    }
    toast(error?.message || "Không tải được danh sách lịch.", "error");
  } finally {
    setButtonLoading(els.viewScheduledWorkOrdersBtn, false);
  }
}
...
function resetWorkSupervisionEmployeeNotificationState() {
  if (state.workSupervisionEmployeeNotificationTimer) {
    window.clearInterval(state.workSupervisionEmployeeNotificationTimer);
  }
  state.workSupervisionEmployeeNotificationTimer = null;
...
function getWorkSupervisionEmployeeReminderContext() {
  // Phiếu lên lịch có thông báo/bộ đếm riêng theo thời gian Admin đặt. Không phát nhắc lại
...
function showWorkSupervisionEmployeeReminder(context) {
...
function runWorkSupervisionEmployeeReminderTick() {
  const context = getWorkSupervisionEmployeeReminderContext();
  if (!context) {
    resetWorkSupervisionEmployeeNotificationState();
    return;
  }
  ...
  showWorkSupervisionEmployeeReminder(context);
}

function ensureWorkSupervisionEmployeeReminderTimer() {
  const context = getWorkSupervisionEmployeeReminderContext();
  if (!context) {
    resetWorkSupervisionEmployeeNotificationState();
    return;
  ...
  return scheduledWorkOrders.flatMap((workOrder) => {
    ...
    return state.employees
      ...
      .map((employee) => ({
        ...
        countdownMinutes: normalizeScheduledCountdownMinutes(workOrder.scheduledAssignmentCountdownMinutes),
...
function renderWorkSupervisionCountdown() {
  ...
  // chính mốc đếm ngược và nhóm đã lưu trên workOrder, không dùng bộ đếm chung.
  ...
  if (showAdmin) {
    ...
    if (els.adminWorkSupervisionMessage) {
      if (scheduledStates.length && !regularStates.length) {
        ...
        const countdownValues = [...new Set(scheduledStates.map((item) => item.countdownMinutes))];
        const countdownText = countdownValues.length === 1
          ? `${countdownValues[0]} phút`
          : "theo thời gian Admin đã đặt";
        els.adminWorkSupervisionMessage.textContent = waitingCount > 0
          ? `${scheduledEmployeeCount} nhân viên đúng nhóm đang chờ nhận Phiếu lên lịch; ${waitingCount} bộ đếm đã hết và đang chờ giao Phiếu.`
          : `${scheduledEmployeeCount} nhân viên thuộc ${groupNames.join(", ")} đang đếm ngược ${countdownText}. Nhân viên nhóm khác không tham gia bộ đếm này.`;
      } else {
        els.adminWorkSupervisionMessage.textContent = waitingCount > 0
  ...
  if (employeeIncluded) {
    ...
    if (els.employeeWorkSupervisionMessage) {
      if (isScheduledEmployeeCountdown) {
        const scheduledCountdownMinutes = normalizeScheduledCountdownMinutes(data.countdownMinutes);
        els.employeeWorkSupervisionMessage.textContent = waitingForServer
          ? `Bộ đếm ${scheduledCountdownMinutes} phút của Phiếu “${data.workOrderName}” dành cho nhóm ${data.groupName} đã hết. Hệ thống đang chờ Admin giao Phiếu.`
          : `Bạn thuộc nhóm ${data.groupName} được chọn cho Phiếu lên lịch “${data.workOrderName}”. Nhân viên ngoài nhóm không tham gia bộ đếm ${scheduledCountdownMinutes} phút này.`;
      } else {
        els.employeeWorkSupervisionMessage.textContent = waitingForServer
...
setInterval(renderWorkSupervisionCountdown, 1000);
...
window.addEventListener("focus", ensureWorkSupervisionEmployeeReminderTimer);
  ...
  return `
    <article class="task-card ${taskCardClass(displayStatus)} ${attentionClass} ${mobileExpandedClass} ${desktopExpandedClass}" data-task-card data-task-id="${escapeHtml(task.id)}" data-work-order-id="${escapeHtml(task.workOrderId || "legacy")}" data-deadline-ms="${deadlineMs}" data-deadline-minutes="${Number(task.deadlineMinutes || 0)}" data-queue-start-ms="${queueStartMs}" data-remaining-pause-ms="${remainingPauseMs}" data-raw-status="${escapeHtml(task.status)}" data-display-status="${escapeHtml(displayStatus)}">
      ...
      <div class="task-mobile-details" id="${escapeHtml(mobileDetailsId)}">
        ...
        ${renderWorkSupervisionLunchCreditBox(task)}
...
function renderWorkSupervisionLunchCreditBox(task) {
  const initialMinutes = Math.max(0, Number(task?.workSupervisionInitialMinutes || 0));
  if (!isLunchBreakTask(task) || initialMinutes <= 0) return "";
...
function renderPhotoReportBox(task, mode) {
  ...
  const editingLockedAfterDispatch = mode === "admin"
    ...
    && task.scheduledWorkOrder !== true
    && isDispatchedPhotoRequirementEditingLocked(task);
  const editingLockedBySchedule = mode === "admin"
    && hasPermission("managePhotoRequirements")
    && task.scheduledWorkOrder === true;
  ...
  return `
    ...
    >
      <div>
        ...
        ${editingLockedBySchedule ? `<small class="photo-report-hint">🔒 Số ảnh báo cáo được khóa vì Phiếu được tạo tự động từ Lên lịch.</small>` : ""}
...
async function editTaskPhotoRequirement(taskId) {
  ...
  if (task.scheduledWorkOrder === true) {
    showToast("Phiếu được tạo tự động từ Lên lịch nên số lượng ảnh báo cáo đã bị khóa.");
    return;
  }
...
function updateCountdowns() {
  $$("[data-scheduled-assignment-deadline-ms]").forEach((countdown) => {
    ...
    const countdownMinutes = normalizeScheduledCountdownMinutes(
      countdown.dataset.scheduledAssignmentCountdownMinutes
    );
    const remainingMs = deadlineMs - Date.now();
    countdown.textContent = remainingMs > 0
      ? "Còn " + formatCountdown(remainingMs) + " để giao Phiếu"
      : `Đã hết ${countdownMinutes} phút - chờ giao việc`;
